...
class TestIsBouquetComponent:
    """Tests for is_bouquet_component with both method variants."""


    def test_result_has_all_keys_optimized(self):
        result = is_bouquet_component(nx.path_graph(4))
        assert set(result.keys()) == _EXPECTED_KEYS

    def test_result_has_all_keys_baseline(self):
        result = is_bouquet_component(nx.path_graph(4), method="baseline")
        assert set(result.keys()) == _EXPECTED_KEYS

    def test_method_field_matches_argument(self):
        for m in ("baseline", "optimized"):
            assert is_bouquet_component(nx.path_graph(4), method=m)["method"] == m

    def test_unknown_method_raises_value_error(self):
        with pytest.raises(ValueError, match="Unknown method"):
            is_bouquet_component(nx.path_graph(4), method="mystery")

    def test_default_method_is_optimized(self):
    ...
    def test_tree_returns_not_bouquet(self, method):
        result = is_bouquet_component(nx.path_graph(5), method=method)
    ...
    @pytest.mark.parametrize("method", ["baseline", "optimized"])
    def test_c4_not_bouquet(self, method):
        """C4 has one cycle of length 4, not 5."""
        result = is_bouquet_component(nx.cycle_graph(4), method=method)
        assert result["is_bouquet"] is False
        assert result["cycle_nodes"] is None

    @pytest.mark.parametrize("method", ["baseline", "optimized"])
    def test_c6_not_bouquet(self, method):
        """C6 has one cycle of length 6, not 5."""
        result = is_bouquet_component(nx.cycle_graph(6), method=method)
        assert result["is_bouquet"] is False


    @pytest.mark.parametrize("method", ["baseline", "optimized"])
    def test_valid_bouquet_accepted(self, method):
        result = is_bouquet_component(_build_uniform_bouquet(1), method=method)
    ...
    @pytest.mark.parametrize("method", ["baseline", "optimized"])
    def test_valid_bouquet_deeper_petals(self, method):
        result = is_bouquet_component(_build_uniform_bouquet(2), method=method)
        assert result["is_bouquet"] is True


    @pytest.mark.parametrize("method", ["baseline", "optimized"])
    def test_c5_with_chord_not_bouquet(self, method):
        """C5 with an extra chord: two basis cycles, not a single induced C5."""
        result = is_bouquet_component(_c5_with_chord(), method=method)
        assert result["is_bouquet"] is False

    @pytest.mark.parametrize("method", ["baseline", "optimized"])
    def test_mixed_bouquet_not_bouquet(self, method):
        """C5 with non-isomorphic petals is rejected."""
        result = is_bouquet_component(
            _build_mixed_bouquet([1, 1, 2, 2, 2]), method=method
        )
        assert result["is_bouquet"] is False
    ...
    @pytest.mark.parametrize("method", ["baseline", "optimized"])
    def test_uniform_colors_accepted(self, method):
        G = _build_uniform_bouquet(1)
        labels = {v: 0 for v in G.nodes()}
        result = is_bouquet_component(G, method=method, labels=labels)
        assert result["is_bouquet"] is True

    @pytest.mark.parametrize("method", ["baseline", "optimized"])
    def test_unique_colors_rejected(self, method):
        G = _build_uniform_bouquet(1)
        labels = {v: v for v in G.nodes()}
        result = is_bouquet_component(G, method=method, labels=labels)
        assert result["is_bouquet"] is False
...
class TestCompareBouquetMethods:
    """Tests for compare_bouquet_methods: both variants run, results agree."""

    def test_returns_baseline_and_optimized_keys(self):
        result = compare_bouquet_methods(nx.path_graph(4))
        assert "baseline" in result and "optimized" in result

    def test_each_sub_result_has_all_keys(self):
        result = compare_bouquet_methods(nx.path_graph(4))
        for key in ("baseline", "optimized"):
            assert set(result[key].keys()) == _EXPECTED_KEYS

    def test_both_agree_on_tree(self):
        result = compare_bouquet_methods(nx.path_graph(5))
        assert result["baseline"]["is_bouquet"] is False
        assert result["optimized"]["is_bouquet"] is False

    def test_both_agree_on_non_c5_cycle(self):
        result = compare_bouquet_methods(nx.cycle_graph(4))
        assert result["baseline"]["is_bouquet"] is False
        assert result["optimized"]["is_bouquet"] is False

    def test_both_agree_on_valid_bouquet(self):
        result = compare_bouquet_methods(_build_uniform_bouquet(1))
        assert result["baseline"]["is_bouquet"] is True
        assert result["optimized"]["is_bouquet"] is True

    def test_both_agree_on_neither_tree_nor_bouquet(self):
        """C5 with a chord: multiple cycles, not a unicyclic C5 bouquet."""
        result = compare_bouquet_methods(_c5_with_chord())
        assert result["baseline"]["is_bouquet"] is False
        assert result["optimized"]["is_bouquet"] is False

    def test_both_agree_on_non_isomorphic_petals(self):
        result = compare_bouquet_methods(_build_mixed_bouquet([1, 1, 2, 2, 2]))
        assert result["baseline"]["is_bouquet"] is False
        assert result["optimized"]["is_bouquet"] is False

    def test_method_fields_differ(self):
        result = compare_bouquet_methods(_build_uniform_bouquet(1))
        assert result["baseline"]["method"] == "baseline"
        assert result["optimized"]["method"] == "optimized"
