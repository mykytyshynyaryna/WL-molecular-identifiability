"""
Profile: bouquet detection — OPTIMIZED implementation.

Function profiled : wl_identifiability.bouquet._check_bouquet_component_optimized
Implementation    : materialises a plain adjacency dict once, uses leaf-stripping
                    to find the C5, then compares petals via AHU canonical string
                    signatures — no graph copies, no nx.is_isomorphic.
Compared against  : profile_bouquet_baseline.py (nx.cycle_basis + nx.is_isomorphic)
...
Usage (from project root):
    python profiling/profile_bouquet_optimized.py
    python profiling/profile_bouquet_optimized.py --data data/AAAC.smi --max-mols 500
    python profiling/profile_bouquet_optimized.py --reps 10 --no-save
...
from wl_identifiability.bouquet import _check_bouquet_component_optimized
