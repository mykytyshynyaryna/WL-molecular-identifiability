...
### Summary of bouquet-forest verdicts (4,111 molecules across MUTAG, NCI1, NCI109, ZINC20)

- **`top=1, atom=0` (suspicious):** 0 (0.000000%)
- **`top=0, atom=1` (expected mismatch):** 58 (1.4108%)
- **`top=1, atom=1` (both accept):** 3,499 (85.11%)
- **`top=0, atom=0` (both reject):** 554 (13.4760%)

**Top-level rejection reasons (raw):**
- `cycle_not_5`: 341
- `multiple_cycles_in_component`: 242
- `petals_not_isomorphic`: 29

**Atom-level rejection reasons (raw):**
- `cycle_not_5`: 292
- `multiple_cycles_in_component`: 234
- `petals_not_isomorphic`: 28
...
```
                       top  atom
category                        
not_c5_cycle           341   292
multiple_cycles        242   234
petals_not_isomorphic   29    28
```

**Representative-sample diversity (by `dataset_name`):**

- `top1_atom0`: 0 rows
- `top0_atom1`: 2 rows across 1 datasets (nci109_smiles.smi)
- `top1_atom1`: 2 rows across 1 datasets (nci109_smiles.smi)
- `top0_atom0`: 2 rows across 1 datasets (nci109_smiles.smi)
...
- `top0_atom1` (n=58) is the inverse: top-level rejects, atom-level accepts.
...
Outputs saved to `C:\Users\yaryn\OneDrive\Desktop\Thesis\Is_1_WL_Expressivity_Sufficient_for_Molecular_Graphs\scripts\results\interesting_cases_NCI109`:
